...
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| `@hedwigjs/client`          | SDK for modules: `createClient` / `createRemoteClient`, `whenRuntimeReady`, `hasCapability`, `getRuntimeInfo`, and every type a module can see. No runtime dependency — finds the host's runtime through a per-realm handle (`Symbol.for('@hedwigjs/runtime/1')`) and works before `initBroker()` (lazy client, flushed in order). | [![npm](https://img.shields.io/npm/v/@hedwigjs/client?label=)](https://www.npmjs.com/package/@hedwigjs/client) |
| `@hedwigjs/react`           | Hooks on top of the SDK: `useClient`, `useTopic`, `useStateTopic` (retained value, before first paint when the runtime is already there), `useRequest` (`send` + `pending` / `result`), `useRemoteClient` (a remote for the component's lifetime), `useRuntimeReady`, `bindHooks`. React 18 / 19. | [![npm](https://img.shields.io/npm/v/@hedwigjs/react?label=)](https://www.npmjs.com/package/@hedwigjs/react) |
| `@hedwigjs/vue`             | The same as Vue 3 composables, plus `bindComposables`. | [![npm](https://img.shields.io/npm/v/@hedwigjs/vue?label=)](https://www.npmjs.com/package/@hedwigjs/vue) |
| `@hedwigjs/broker`          | The runtime (host only): `initBroker`, `getBroker`, `createRemoteClient`. Routes messages **in-process** between clients on the same broker and, through **remote clients**, across contexts (iframes, tabs, workers, backends over `postmessage` / `message-port` / `broadcast-channel` / `websocket` / `sse` / your own). Hooks (`beforeSend` / `afterSend` / `onSubscribe`), per-topic retention, backpressure. Ships the wire envelope v1 JSON Schema (`@hedwigjs/broker/spec/envelope-v1.schema.json`) and the transport conformance kit (`@hedwigjs/broker/conformance`). | [![npm](https://img.shields.io/npm/v/@hedwigjs/broker?label=)](https://www.npmjs.com/package/@hedwigjs/broker) |
| `@hedwigjs/devtools`        | React panel (React 18.2 / 19) that mounts inside the host app. Messages, clients (local and remote), replay buffer, dedicated system-events stream.                                                                                                                                                | [![npm](https://img.shields.io/npm/v/@hedwigjs/devtools?label=)](https://www.npmjs.com/package/@hedwigjs/devtools) |
| `@hedwigjs/create-registry` | Optional CLI (`npm create @hedwigjs/registry`) that scaffolds a topic-registry package: one contract file per topic (`kind`, payload, `response`, `retention`) and codegen for `Topic`, `TopicPayloads`, `TopicContracts`, `TOPIC_KINDS`. The broker also accepts topic types from Zod / Protobuf / GraphQL / hand-written — registry is a pattern, not a mandate. | [![npm](https://img.shields.io/npm/v/@hedwigjs/create-registry?label=)](https://www.npmjs.com/package/@hedwigjs/create-registry) |
