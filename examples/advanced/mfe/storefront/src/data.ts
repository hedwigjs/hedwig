 ...
 *
 * Not a topic contract — just the seed data storefront renders when there
 * is no real menu API to call. In a real deployment this would come from
 * `GET /menu` on the backend and this constant would disappear.
 *
 * Lives here (not in `@hedwig-demo/contracts`) because a fixture is
 * storefront's concern, not part of the topic registry. The registry only
