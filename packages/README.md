...
| Directory   | npm name             | Role                                                                                                | npm                     |
| ----------- | -------------------- | --------------------------------------------------------------------------------------------------- | ----------------------- |
| `client/`   | `@hedwigjs/client`   | SDK for modules: `createClient`, `createRemoteClient`, `whenRuntimeReady`, `hasCapability`, `getRuntimeInfo`, all module-visible types. No runtime dependency; locates the host's runtime via `Symbol.for('@hedwigjs/runtime/1')` and hands out a lazy client until it appears. | [![npm](https://img.shields.io/npm/v/@hedwigjs/client?label=)](https://www.npmjs.com/package/@hedwigjs/client) |
| `react/`    | `@hedwigjs/react`    | React 18 / 19 hooks on the SDK: `useClient`, `useTopic`, `useStateTopic`, `useRequest`, `useRemoteClient`, `useRuntimeReady`, `bindHooks`. | [![npm](https://img.shields.io/npm/v/@hedwigjs/react?label=)](https://www.npmjs.com/package/@hedwigjs/react) |
| `vue/`      | `@hedwigjs/vue`      | Vue 3 composables with the same surface, plus `bindComposables`. | [![npm](https://img.shields.io/npm/v/@hedwigjs/vue?label=)](https://www.npmjs.com/package/@hedwigjs/vue) |
...
| `devtools/` | `@hedwigjs/devtools` | React panel (React 18.2 / 19): message timeline, clients (local and remote), replay buffer, system events. | [![npm](https://img.shields.io/npm/v/@hedwigjs/devtools?label=)](https://www.npmjs.com/package/@hedwigjs/devtools) |
